...
public class ReplicationConnectionProxy extends MultiHostConnectionProxy implements PingTarget {
   ...
   private ReplicationConnectionProxy(ReplicationConnectionUrl connectionUrl) throws SQLException {
      ...
      try {
         ...
         throw SQLError.createSQLException(Messages.getString("ReplicationConnectionProxy.badValueForAllowMasterDownConnections", new Object[]{enableJMXAsString}), "S1009", (ExceptionInterceptor)null);
      }
      ...
      try {
         ...
         throw SQLError.createSQLException(Messages.getString("ReplicationConnectionProxy.badValueForAllowSlaveDownConnections", new Object[]{allowSlaveDownConnectionsAsString}), "S1009", (ExceptionInterceptor)null);
      }
      ...
      try {
         ...
         throw SQLError.createSQLException(Messages.getString("ReplicationConnectionProxy.badValueForReadFromMasterWhenNoSlaves", new Object[]{readFromMasterWhenNoSlavesAsString}), "S1009", (ExceptionInterceptor)null);
      }

      String group = props.getProperty("replicationConnectionGroup", (String)null);
      if (group != null) {
   ...
   private boolean switchToSlavesConnectionIfNecessary() throws SQLException {
      return this.currentConnection != null && (!this.isMasterConnection() || !this.readOnly && (!this.masterHosts.isEmpty() || !this.currentConnection.isClosed())) && (this.isMasterConnection() || !this.currentConnection.isClosed()) ? false : this.switchToSlavesConnection();
   }
   ...
   public boolean isHostMaster(String hostPortPair) {
      return hostPortPair == null ? false : this.masterHosts.stream().anyMatch((hi) -> {
   ...
   public boolean isHostSlave(String hostPortPair) {
      return hostPortPair == null ? false : this.slaveHosts.stream().anyMatch((hi) -> {
         return hostPortPair.equalsIgnoreCase(hi.getHostPortPair());
      });
   }
   ...
   private HostInfo getMasterHost(String hostPortPair) {
      return (HostInfo)this.masterHosts.stream().filter((hi) -> {
         return hostPortPair.equalsIgnoreCase(hi.getHostPortPair());
      }).findFirst().orElse((Object)null);
   }

   private HostInfo getSlaveHost(String hostPortPair) {
      return (HostInfo)this.slaveHosts.stream().filter((hi) -> {
         return hostPortPair.equalsIgnoreCase(hi.getHostPortPair());
      }).findFirst().orElse((Object)null);
   }

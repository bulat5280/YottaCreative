...
public class ConnectionWrapper extends WrapperBase implements JdbcConnection {
   ...
   protected void close(boolean fireClosedEvent) throws SQLException {
      synchronized(this.pooledConnection) {
         if (!this.closed) {
            if (!this.isInGlobalTx() && (Boolean)this.mc.getPropertySet().getBooleanReadableProperty("rollbackOnPooledClose").getValue() && !this.getAutoCommit()) {
               this.rollback();
            }
   ...
   public ResultSetInternalMethods execSQL(StatementImpl callingStatement, String sql, int maxRows, PacketPayload packet, boolean streamResults, String catalog, ColumnDefinition cachedMetadata, boolean isBatch) throws SQLException {
      try {
         ...
         throw SQLExceptionsMapping.translateException(var10, super.exceptionInterceptor);
      }
   }

...
public class Rotation implements Serializable {
   ...
   public double getAngle() {
      if (!(this.q0 < -0.1D) && !(this.q0 > 0.1D)) {
         return this.q0 < 0.0D ? 2.0D * FastMath.acos(-this.q0) : 2.0D * FastMath.acos(this.q0);
      } else {
   ...
   public double[] getAngles(RotationOrder order) throws CardanEulerSingularityException {
      ...
      if (order == RotationOrder.XYZ) {
         v1 = this.applyTo(Vector3D.PLUS_K);
         v2 = this.applyInverseTo(Vector3D.PLUS_I);
         ...
         v1 = this.applyTo(Vector3D.PLUS_J);
         v2 = this.applyInverseTo(Vector3D.PLUS_I);
         ...
         v1 = this.applyTo(Vector3D.PLUS_K);
         v2 = this.applyInverseTo(Vector3D.PLUS_J);
         if (!(v2.getZ() < -0.9999999999D) && !(v2.getZ() > 0.9999999999D)) {
         ...
         v1 = this.applyTo(Vector3D.PLUS_I);
         v2 = this.applyInverseTo(Vector3D.PLUS_J);
         ...
         v1 = this.applyTo(Vector3D.PLUS_J);
         v2 = this.applyInverseTo(Vector3D.PLUS_K);
         if (!(v2.getY() < -0.9999999999D) && !(v2.getY() > 0.9999999999D)) {
   ...
   public void applyTo(double[] in, double[] out) {
      ...
      double z = in[2];
      double s = this.q1 * x + this.q2 * y + this.q3 * z;
      out[0] = 2.0D * (this.q0 * (x * this.q0 - (this.q2 * z - this.q3 * y)) + s * this.q1) - x;
      out[1] = 2.0D * (this.q0 * (y * this.q0 - (this.q3 * x - this.q1 * z)) + s * this.q2) - y;
      out[2] = 2.0D * (this.q0 * (z * this.q0 - (this.q1 * y - this.q2 * x)) + s * this.q3) - z;
   }
   ...
   public void applyInverseTo(double[] in, double[] out) {
      ...
      double s = this.q1 * x + this.q2 * y + this.q3 * z;
      double m0 = -this.q0;
      out[0] = 2.0D * (m0 * (x * m0 - (this.q2 * z - this.q3 * y)) + s * this.q1) - x;
      out[1] = 2.0D * (m0 * (y * m0 - (this.q3 * x - this.q1 * z)) + s * this.q2) - y;
      out[2] = 2.0D * (m0 * (z * m0 - (this.q1 * y - this.q2 * x)) + s * this.q3) - z;
   }

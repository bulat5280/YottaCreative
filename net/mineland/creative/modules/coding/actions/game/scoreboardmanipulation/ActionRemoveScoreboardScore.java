...
public class ActionRemoveScoreboardScore extends Action {
   public ActionRemoveScoreboardScore(Activator activator) {
      super(activator);
   }

   public ActionType getType() {
      return ActionType.GAME_REMOVE_SCOREBOARD_SCORE;
   }
   ...
   public ItemData getIcon() {
      return new ItemData(Material.INK_SACK);
   }
   ...
   public boolean run(List<Entity> selectedEntities, GameEvent gameEvent, int stackCounter, AtomicInteger callCounter) {
      selectedEntities.forEach((entity) -> {
         ...
         int score = this.getVar("score", 0, gameEvent, entity);
         PlotScoreboard scoreboard = gameEvent.getPlot().getCodeHandler().getScoreboard(name);
         if (scoreboard != null) {
            if (tag != null) {
               scoreboard.removeScore(tag);
            } else {
               scoreboard.removeScore(score);
            }
         }
   ...
   public boolean parseChest(ChestParser chestParser) {
      ...
      this.putVar("score", chestParser.getNumber(0));
      return true;
   }
}

...
public class ActionIfVariableTextEquals extends ActionIf {
   public ActionIfVariableTextEquals(Activator activator) {
      super(activator);
   }
   ...
   public boolean expression(List<Entity> selectedEntities, GameEvent gameEvent) {
      ...
      do {
         ...
         string = object instanceof Double ? object.toString().replace(".0", "") : object.toString();
         text = this.getVarString("text", gameEvent, entity);
         if (text == null && dynamicVariable.getValue(gameEvent.getPlot()) != null) {
            return false;
         }
      } while(text == null || text.equalsIgnoreCase(string));

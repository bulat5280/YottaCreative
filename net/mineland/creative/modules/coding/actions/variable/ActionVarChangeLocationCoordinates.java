...
public class ActionVarChangeLocationCoordinates extends Action {
   public ActionVarChangeLocationCoordinates(Activator activator) {
      super(activator);
   }

   public ActionType getType() {
      return ActionType.VARIABLE_CHANGE_LOCATION_COORDINATES;
   }
   ...
   public boolean run(List<Entity> selectedEntities, GameEvent gameEvent, int stackCounter, AtomicInteger callCounter) {
      ...
      while(var5.hasNext()) {
         ...
         double x = location1.getX() + ((Number)numbers.get(i)).doubleValue();
         double y = location1.getY() + ((Number)numbers.get(i + 1)).doubleValue();
         double z = location1.getZ() + ((Number)numbers.get(i + 2)).doubleValue();

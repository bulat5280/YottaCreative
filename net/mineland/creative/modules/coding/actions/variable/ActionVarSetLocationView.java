...
public class ActionVarSetLocationView extends Action {
   public ActionVarSetLocationView(Activator activator) {
      super(activator);
   }

   public ActionType getType() {
      return ActionType.VARIABLE_SET_LOCATION_VIEW;
   }
   ...
   public boolean run(List<Entity> selectedEntities, GameEvent gameEvent, int stackCounter, AtomicInteger callCounter) {
      ...
      while(var5.hasNext()) {
         ...
         float yaw = ((Number)numbers.get(i)).floatValue();
         float pitch = ((Number)numbers.get(1 + i)).floatValue();

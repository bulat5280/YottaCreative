...
public class GuiMenuWorldCode extends GuiMenu {
   private static ModuleCreative moduleCreative = (ModuleCreative)Module.getInstance(ModuleCreative.class);

   public GuiMenuWorldCode(String key, User user) {
      super(key, user, 6, Message.getMessage((IUser)user, "creative.gui.world_settings.code.title"));
      this.init();
   }
   ...
   private void init() {
      ...
      if (plot != null) {
         List<User> allowed = (List)plot.getOnlinePlayers().stream().filter((u) -> {
            return plot.getCodingRegion().getMembers().contains(u.getName()) && !u.getName().equals(this.getUser().getName());
         ...
         for(var5 = allowed.iterator(); var5.hasNext(); ++pos) {
            user = (User)var5.next();
            this.addItem(pos, ItemStackUtil.createSkinSkullByUser(user.getName()), Message.getMessage((IUser)this.getUser(), "creative.gui.world_settings.code.allowed", "{player}", user.getPlayer().getDisplayName()), (event) -> {
               this.getUser().getPlayer().chat("/dev " + user.getPlayer().getName());
               this.init();
         ...
         for(var5 = denied.iterator(); var5.hasNext(); ++pos) {
            user = (User)var5.next();
            this.addItem(pos, ItemStackUtil.createSkinSkullByUser(user.getName()), Message.getMessage((IUser)this.getUser(), "creative.gui.world_settings.code.denied", "{player}", user.getPlayer().getDisplayName()), (event) -> {
               this.getUser().getPlayer().chat("/dev " + user.getPlayer().getName());
               this.init();
            });
         }
